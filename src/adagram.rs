...
impl VectorModel {
    ...
    pub fn load_model(
        path: &str,
    ) -> Result<(VectorModel, Vec<String>), Box<dyn std::error::Error>> {
        let mut rf = std::io::BufReader::new(std::fs::File::open(path)?);
        ...
        for i in 0..lexsize {
            ...
            let hw_nsenses = line
                .trim()
                .parse::<usize>()
                .map_err(|_| "bad number of senses")?;
        ...
        F: Fn(u32) -> String,
    {
        ...
        for &e in self.freqs.iter() {
            vecwr.write_all(&e.to_le_bytes())?;
        }
        for &e in self.code.iter() {
            vecwr.write_all(&e.to_le_bytes())?;
        }
    ...
    pub fn nmeanings(&self) -> usize {
        self.in_vecs.shape()[1]
    }

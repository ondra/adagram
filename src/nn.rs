...
impl VectorModel {
    ...
    pub fn norm(&mut self) {
        ...
        for i in 0..ii {
            for j in 0..jj {
                let mut v = self.in_vecs.slice_mut(s![i as usize, j, ..]);
                let n = norm_l2(&v);
...
pub fn sim_normed(vm: &VectorModel,
            ...
            head_id2: usize, senseno2: usize) -> f32
{
    let qvec1 = vm.in_vecs.slice(s![head_id1 as usize, senseno1, ..]);
    let qvec2 = vm.in_vecs.slice(s![head_id2 as usize, senseno2, ..]);
    qvec1.dot(&qvec2)
}

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.print_status {
        colnames.push("status".to_string());
    }
    colnames.push("cluster".to_string());
    if args.print_probs {
        colnames.push("cluster_probs".to_string())
    };
    if args.print_nsenses {
        colnames.push("nsenses".to_string())
    };
    ...
    for maybeline in lines {
        ...
        if let Some(mp) = maxpos {
            outvals.push(format!("c{}", mp))
        } else {
            outvals.push("-1".to_string())
        }
    ...
    if headword_not_in_lexicon > 0 || ctx_empty > 0 || ctx_all_unknown > 0 || allok == 0 {
        eprintln!(
            "encountered {} headwords not present in lexicon",
            headword_not_in_lexicon
        );
        eprintln!(
            "encountered {} lines where no context words were present",
            ctx_empty
        );
        eprintln!(
            "encountered {} lines where no context words were found in lexicon",
            ctx_all_unknown
        );
    }
